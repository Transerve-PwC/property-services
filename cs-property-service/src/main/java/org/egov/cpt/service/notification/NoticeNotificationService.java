...
public class NoticeNotificationService {
	...
	@Autowired
	public NoticeNotificationService(PropertyConfiguration config, ServiceRequestRepository serviceRequestRepository,
	...
	private void enrichEMAILRequest(NoticeGenerationRequest request,List<Property> propertiesFromDb, List<EmailRequest> emailRequest) {
	        ...
	        for(NoticeGeneration notice : request.getNoticeApplications()){
	        	...
	        	/* for(Property property:propertiesFromDb){
	            	...
	            	for(Owner owner:owners){
	            		if(owner.getOwnerDetails().getName().equals(property.getPropertyDetails().getCurrentOwner())){
	...
	private void enrichSMSRequest(NoticeGenerationRequest request,List<Property> propertiesFromDb, List<SMSRequest> smsRequests) {
		String tenantId = request.getNoticeApplications().get(0).getTenantId();
		for (NoticeGeneration notice : request.getNoticeApplications()) {
			String message = null;
			String localizationMessages;
			...
			if (ownerDtl.getOwnerDetails().getPhone() != null) {
				mobileNumberToOwner.put(ownerDtl.getOwnerDetails().getPhone(),
						ownerDtl.getOwnerDetails().getName());
			}
