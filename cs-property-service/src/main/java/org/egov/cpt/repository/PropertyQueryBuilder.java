...
public class PropertyQueryBuilder {
	...
	private static final String FINANCE_COLUMNS =  "demand.id as demand_id,demand.property_id as demand_pid,demand.initialGracePeriod as demand_IniGracePeriod, demand.generationDate as demand_genDate,"
											...
											+ "account.id as account_id,account.payment_id as account_pid,account.remainingAmount as account_remainingAmount,account.tenantid as account_tenantid,"
	...
	private static final String OWNER_TABLE = " cs_pt_ownership_v1 ownership ON pt.id=ownership.property_id " + LEFT_JOIN
	...
	private static final String NOTICE_TABLE = " cs_pt_notice_generation_application ng ON ng.propertyid = pt.id " + LEFT_JOIN
											+ " cs_pt_documents_v1 ngdoc ON ngdoc.reference_id =  ng.id ";
...
//		StringBuilder builder = new StringBuilder(SEARCH_QUERY);
		...
		if (relations == null) {
			...
			builder.append(PT_COLUMNS + "," + OWNER_COLUMNS + "," + PI_COLUMNS + "," + NOTICE_COLUMNS+" , "+GD_COLUMNS+" , "+FINANCE_COLUMNS);
			builder.append(PT_TABLE + LEFT_JOIN + OWNER_TABLE + LEFT_JOIN + PI_TABLE + LEFT_JOIN + NOTICE_TABLE + LEFT_JOIN + GD_TABLE + LEFT_JOIN + FINANCE_TABLE );
		} 
		else {
