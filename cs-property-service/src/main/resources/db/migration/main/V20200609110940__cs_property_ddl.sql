...
DROP TABLE IF EXISTS cs_pt_correspondence_address_v1;
...
CREATE TABLE cs_pt_correspondence_address_v1 (
   id           		CHARACTER VARYING (256) NOT NULL,
   property_id       	CHARACTER VARYING (256),
   transit_number       CHARACTER VARYING (256),
   tenantid			    CHARACTER VARYING (256),
  ...
  CONSTRAINT pk_cs_pt_correspondence_address_v1 PRIMARY KEY (id),
  CONSTRAINT fk_cs_pt_correspondence_address_v1 FOREIGN KEY (property_id) REFERENCES cs_pt_property_v1 (id)
  ON UPDATE CASCADE
  ON DELETE CASCADE
);
