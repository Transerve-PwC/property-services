...
//		TODO: change in mdms-data config
	...
	public List<Map<String, Object>> getNotificationConfig(String applicationType, RequestInfo requestInfo, String tenantId, Application application) {
		// TODO Auto-generated method stub
		tenantId = tenantId.split("\\.")[0];
		//String filter = "[?(@.state=='EM_ES_PENDING_CLERK_VERIFICATION'&&@.applicationType=='SaleDeed'&&@.moduleType=='OwnershipTransfer')]";
		String filter = "[?(@.state=='"+application.getState()+"'&&@.applicationType=='"+application.getApplicationType()+"'&&@.moduleType=='"+application.getModuleType()+"')]";
		MdmsCriteriaReq mdmsCriteriaReq = new MdmsCriteriaReq();
			mdmsCriteriaReq = util.prepareMdMsRequest(tenantId, PSConstants.MDMS_PS_MODULE_NAME,
					Arrays.asList(applicationType), filter, requestInfo);
